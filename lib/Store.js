...
function parseEvents(text) {
  ...
  for (var i = 0; i < parsed.events.length; i++) {
    ...
    out.push({
      // The clock time as the calendar wrote it. Re-parsing into a Date would
      // re-interpret the offset the ICS already resolved.
      at: e.all_day ? "" : start.slice(11, 16),
      title: String(e.title || e.summary || "(untitled)"),
      color: String(e.calendar_color || e.color || "")
    })
  }
  // All-day first, then by clock time: an all-day event has no place in a
  // timed ordering, and belongs above the day rather than buried in it.
