...
// A task is a container -- it carries the note, the screenshots and the
// subtasks -- so emptying its text is not a request to throw all of that away.
// A subtask holds only its text, which is why setSubText's caller may treat an
// empty value as a removal and this one may not.
function setTaskText(day, id, text) {
  var value = String(text === null || text === undefined ? "" : text).trim()
  if (!value) return day
  return updateTask(day, id, function(t) { t.text = value; return t })
}

// Takes a task rather than a day, because it is reached through
// mutateSelected() where the task is already in hand.
//
// Emptying a subtask means removing it, but removal has to go through the
// overlay so that `u` can put it back -- so this declines to write an empty
// value and leaves that decision to the caller.
function setSubText(task, at, text) {
  if (at < 0 || at >= task.subs.length) return task
  var value = String(text === null || text === undefined ? "" : text).trim()
  if (value) task.subs[at].text = value
  return task
}

function removeSub(day, id, at) {
  return updateTask(day, id, function(t) {
...
// The index matters: a subtask list reads as a sequence of steps, so putting a
// restored one on the end instead of back where it was would quietly rewrite
// what the list says.
function restoreSub(day, id, sub, at) {
  return updateTask(day, id, function(t) {
    var where = Math.max(0, Math.min(at, t.subs.length))
    t.subs.splice(where, 0, { text: sub.text, done: sub.done === true })
    return t
  })
}
